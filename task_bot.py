...
def persistent_kb():
    """Клавиатура которая всегда висит внизу — как в budget-bot."""
...
def add_task(creator, assignee, category, text, deadline):
    with db_connect() as conn:
        cur = conn.execute(
...
def main_menu_inline_kb():
    """Инлайн-кнопки внутри сообщения."""
...
async def show_main_menu(target, name: str):
    """Показать главное меню — работает и для message и для callback query."""
...
# ─── /start и кнопка 🏠 Главное меню ────────────────────────────────────────
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ctx.user_data.clear()
    user_id = update.effective_user.id
    name = who_am_i(user_id)
    if name:
        # Сначала показываем/обновляем постоянную клавиатуру внизу
        await update.message.reply_text(
            "·",  # невидимый текст-якорь для клавиатуры
            reply_markup=persistent_kb(),
        )
        # Затем показываем само меню
        await update.message.reply_text(
            f"Привет, <b>{name}</b>! 👋\nЧто будем делать?",
            parse_mode="HTML",
...
async def btn_home(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Обработчик кнопки 🏠 Главное меню с клавиатуры."""
    ctx.user_data.clear()
    user_id = update.effective_user.id
    name = who_am_i(user_id)
    if name:
...
async def msg_task_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Игнорируем нажатие кнопки клавиатуры внутри диалога
    if update.message.text == "🏠 Главное меню":
        ctx.user_data.clear()
        name = who_am_i(update.effective_user.id)
        await update.message.reply_text(
            f"Привет, <b>{name}</b>! Что будем делать?",
            parse_mode="HTML",
            reply_markup=main_menu_inline_kb(),
        ...
        if assignee_id:
            ...
            await ctx.bot.send_message(
                ...
                text=(
                    ...
                    f"📅 Дедлайн: {dl_str}\n\n"
                    f"Нажми 🏠 или /start чтобы открыть меню"
                ),
                parse_mode="HTML",
            )
    await q.edit_message_text(
...
async def cb_my_tasks(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    rows = []
    for t in tasks:
        rows.append([InlineKeyboardButton(f"☐ #{t['id']}", callback_data=f"toggle_{t['id']}")])
    rows.append([
...
async def cb_toggle_task(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for t in tasks:
        ...
        rows.append([InlineKeyboardButton(f"{checked} #{t['id']} {t['text'][:30]}", callback_data=f"toggle_{t['id']}")])
    rows.append([
        InlineKeyboardButton("✅ Подтвердить выполнение", callback_data="confirm_complete"),
        InlineKeyboardButton("🏠 Меню", callback_data="main_menu"),
    ])
    ...
    for task_id in pending:
        ...
        if t and t["assignee"] == name:
            ...
            if creator_id and t["creator"] != name:
                await ctx.bot.send_message(
                    chat_id=creator_id,
                    text=(f"✅ <b>{name}</b> выполнил(а) задачу!\n\n<b>{t['text']}</b>"),
                    parse_mode="HTML",
                )
...
async def cb_delete_mine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("Выбери задачу для удаления:", reply_markup=InlineKeyboardMarkup(rows))
...
async def send_notifications(bot):
    ...
    for t in tasks:
        ...
        if total_sec < 0:
            ...
            if (h == 9 and m < 15) or (h == 20 and m < 15):
                send = True
                ...
                try:
                    await bot.send_message(
                        ...
                        text=(
                            ...
                            f"👤 от {t['creator']}\n\n"
                            f"{tag}\n\n"
                            f"Нажми 🏠 внизу чтобы открыть меню"
                        ),
                        parse_mode="HTML",
                    )
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(cb_new_task, pattern="^new_task$"),
        ],
        states={
            STATE_CATEGORY: [
        ...
        fallbacks=[
            ...
            # Кнопка 🏠 с клавиатуры работает как выход из любого состояния
            MessageHandler(filters.Regex("^🏠 Главное меню$"), btn_home),
        ],
        per_user=True,
        per_chat=True,
        allow_reentry=True,
    ...
    # Кнопка 🏠 должна быть зарегистрирована ДО conv чтобы работала вне диалога
    app.add_handler(MessageHandler(filters.Regex("^🏠 Главное меню$"), btn_home), group=0)
